...
extern keybind_struct *keybind_add(SDL_Scancode key, SDL_Keymod mod, const char *command);
extern void keybind_edit(size_t i, SDL_Scancode key, SDL_Keymod mod, const char *command);
extern void keybind_remove(size_t i);
extern void keybind_repeat_toggle(size_t i);
extern char *keybind_get_key_shortcut(SDL_Scancode key, SDL_Keymod mod, char *buf, size_t len);
...
extern void text_get_width_height(font_struct *font, const char *text, uint64_t flags, SDL_Rect *box, int *w, int *h);
